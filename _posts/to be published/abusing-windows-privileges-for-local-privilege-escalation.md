...
最后，由于已经获取了 SYSTEM 权限，则调用 `ExploitTcbPrivilege()` 函数将通过 `RegCreateKeyExW()` API 在 `Image File Execution Options` 注册表下创建一个子项，然后用 `RegSetValueExW()` API 为粘滞键（sethc.exe）设置 Debugger 键实现映像劫持，实现粘滞键后门，如下所示。

```c++
void ExploitTcbPrivilege()
{
	DWORD lResult;
	HKEY hKey;
...
![image-20230704165203268](C:\Users\whoami\AppData\Roaming\Typora\typora-user-images\image-20230704165203268.png)
...
![image-20230704165546403](C:\Users\whoami\AppData\Roaming\Typora\typora-user-images\image-20230704165546403.png)
...
因此这里可以利用与滥用 SeTcbPrivilege 特权时相同的方法。回忆前文中的令牌模拟规则，即使没有 SeImpersonatePrivilege 特权，用户也可以模拟令牌，只要令牌是针对同一用户的，并且完整性级别小于或等于当前进程完整性级别。令牌的完整性级别可以在构造令牌时设置。
